defmodule PhoenixWithPrettyDebugger.Plug.PrettyDebugger.Highlighter do
  @moduledoc false
  alias Makeup.Lexer
  alias Makeup.Registry
  # The ElixirLexer is treated a bit specially because it's always
  # used to highlight function parameters
  ...
  def get_snippet(file, line) do
    if File.regular?(file) and is_integer(line) do
      ...
      # Test if we can lex the file
      case lexer do
        {lexer_module, _options} ->
          get_highlighted_snippet(file, lexer_module, line)
  ...
  defp format_line({nr, tokens, highlight}) do
    ...
    #   |> Enum.join("")
    html = HTMLFormatter.format_inner_as_iolist(tokens, [])
    {nr, html, highlight}
  end
  ...
  defp lexer_for_file(file) do
    ...
    Registry.get_lexer_by_extension(extension)
  end
